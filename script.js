...
setInterval(() => {
  ...
  logo.style.textShadow = `0 0 20px #${
    Math.random() > 0.5 ? "00ffff" : "ff0040"
  }`;
}, 2000);
  ...
  updateParticles() {
    this.particles.forEach((particle) => {
      ...
      if (distance < this.config.mouseDistance) {
        const force =
          (this.config.mouseDistance - distance) / this.config.mouseDistance;
    ...
    for (let i = 0; i < this.particles.length; i++) {
      for (let j = i + 1; j < this.particles.length; j++) {
        ...
        if (distance < this.config.lineDistance) {
          const opacity =
            ((this.config.lineDistance - distance) / this.config.lineDistance) *
